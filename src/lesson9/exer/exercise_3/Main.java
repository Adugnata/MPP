package lesson9.exer.exercise_3;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //sorting code here
        List<Account> sorted = accounts
                .stream()
                .sorted(Comparator.comparing(Account::getBalance).thenComparing(Account::getOwnerName))
                .toList();
        sorted.forEach(System.out::println);
//accounts.stream().map(Account::ge).max(Comparator.comparing(A))
        double s = accounts.stream().map(Account::getBalance).reduce(0, Integer::sum);
